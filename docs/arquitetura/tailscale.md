...
O Argo CD entra só no passo seguinte. O DNS interno é um serviço do host, não do cluster, porque o cluster não tem como oferecer a porta 53 no endereço da tailnet sem um encaminhamento pelo firewalld que passaria por fora do que o Cilium controla. O que o Argo CD declara é o que vem depois de o nome resolver: o [ingress](ingress.md), um Traefik escutando no node pelas portas 80 e 443, que roteia `argocd.guesant.internal`, `keycloak.guesant.internal` e os demais nomes para o `Service` certo, com certificados de uma CA interna. A zona `tailscale` do firewall libera `http` e `https` para ele, e nada além disso.
