...
| Rede | Listas de liberação | Cada namespace só fala com o que usa; o Cilium aplica de verdade (`policyEnforcementMode: always`, `policyAuditMode: false`), não só audita | `argocd/apps/platform/network-policies`, `cilium-egress.yaml` do blog, `ansible/roles/cilium/templates/values.yaml.j2`, Hubble |
| Cluster | Benchmark | kube-bench semanal: o `CronJob` do cluster cobre as checagens de `policies` (RBAC, Pod Security, rede) sem root e sem acesso a `Secret`; um timer no host, rodando como root sob `ProtectSystem=strict` porque precisa ler o processo do k3s, cobre master, etcd, control plane e node | `argocd/apps/platform/kube-bench`, role `kube_bench` |
