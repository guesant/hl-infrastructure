...
| Node | Segredos em repouso | `Secret` do Kubernetes cifrados no datastore do k3s, com recifragem dos que já existiam, a partir do próximo `just bootstrap` | `secrets-encryption` em `ansible/roles/k3s/templates/config.yaml.j2`, `k3s secrets-encrypt status` checado pela role |
| Node | Superfície | kubeconfig do node só para root, `rpcbind` parado e mascarado, a partir do próximo `just bootstrap` | roles `k3s` e `os_prerequisites` |
