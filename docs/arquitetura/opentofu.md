...
<!-- source-of-trust paths="tofu .tools/cloudflare-tunnel-token.sh" -->
...
`tofu/cloudflare/cloudflare.sops.env` guarda, cifrados, o API token e a passphrase do state. `just tofu-cloudflare` roda `sops exec-env` sobre esse arquivo, que decifra os valores só no ambiente do processo filho, e o filho é um `docker run -e CLOUDFLARE_API_TOKEN -e TF_VAR_state_passphrase` com a imagem oficial do OpenTofu: passar `-e NOME` sem valor faz o Docker herdar a variável do ambiente, então o segredo não aparece na linha de comando nem toca o disco. A decifragem acontece no host, e não dentro do container, pelo mesmo motivo de `sops-edit` e `sops-sync`: a identidade de rotina do operador vive na Secure Enclave do Mac e não funciona fora dele.
