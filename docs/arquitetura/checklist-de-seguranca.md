...
| RBAC com privilégio mínimo e sem conceder criação de roles | Parcial | Revisado em 2026-09-15, com uma exceção encontrada depois, em 17 de setembro: curinga só no `argocd-application-controller` (aplica qualquer recurso, por desenho do GitOps), no `argocd-server` (ações da interface), no sops-secrets-operator (cria `Secret` em qualquer namespace) e nos componentes do k3s; o kube-bench roda com ClusterRole só de leitura; o Portainer é a exceção maior do lote, com uma `ClusterRoleBinding` para `cluster-admin`, risco aceito porque o próprio propósito dele é gerenciar qualquer recurso do cluster pela interface, sem uma lista fixa de namespaces ou tipos de recurso para restringir; a revisão periódica confere se aparece curinga novo | K8, KA, SR, SE |
...
| `default-deny` de entrada e saída, liberando só o necessário | Atende | O Cilium nega tudo por padrão (`enable-policy: always`), cada namespace tem uma `CiliumNetworkPolicy` com o que usa, e `policyAuditMode: false` desde 17 de setembro de 2026: as regras bloqueiam de verdade | K8, KA, SR, SE |
...
| Saída e DNS controlados contra vazamento de dados | Atende | Cada namespace só sai para o CoreDNS, para o API server e para as portas externas de que precisa (443 no Argo CD e no app do blog, 7844 e 443 no cloudflared), e isso já é aplicado de verdade desde a virada do modo auditoria | SE |
...
| `requests` e `limits` de memória nos workloads | Atende | Blog, Postgres, Argo CD, cert-manager, CNPG, sops-secrets-operator, Kargo e Cilium com `requests` e limite de memória; CoreDNS, metrics-server e local-path-provisioner são addons do k3s e ficam com os valores dele | K8, KA, SR, SE |
...
| Imagem referenciada por digest ou assinatura verificada | Atende | Tudo fora do `kube-system` por digest, imposto na admissão; o blog promovido pelo Kargo com um `Warehouse` de estratégia `Digest`; o `kube-system` fica de fora por causa do CoreDNS e do local-path-provisioner embutidos no k3s, que o k3s atualiza junto do binário | CP, K8 |
...
A política de rede já bloqueia. Desde 15 de setembro de 2026 cada namespace tem uma `CiliumNetworkPolicy` com o que usa, o Hubble parou de mostrar veredictos `AUDIT` logo depois delas entrarem (com uma exceção achada numa coleta de 14 minutos no mesmo dia, o ping do host para o endpoint de health do próprio Cilium, liberado pela `CiliumClusterwideNetworkPolicy` `cilium-health`), e em 17 de setembro de 2026 `policyAuditMode` virou `false` em `ansible/roles/cilium/templates/values.yaml.j2`: as regras agora bloqueiam de verdade, não só registram. No meio da virada apareceu um efeito colateral do padrão de reaplicar o chart sem um release de Helm de verdade (`helm template` em vez de `helm upgrade`): a função `lookup` que o chart usa pra reaproveitar a CA do Hubble sempre volta vazia nesse modo, então cada reaplicação regenerava a CA e os certificados do zero; a correção foi trocar `hubble.tls.auto.method` de `helm` para `cronJob`, que resolve isso do lado do servidor em vez de em tempo de render (ver [Helm e os charts](helm-e-charts.md)). Zero flows `DROPPED` inesperados na primeira observação depois da virada.
