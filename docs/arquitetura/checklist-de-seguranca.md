...
| `default-deny` de entrada e saída, liberando só o necessário | Atende | O Cilium nega tudo por padrão (`enable-policy: always`), cada namespace tem uma `CiliumNetworkPolicy` com o que usa, e `policyAuditMode: false`: as regras bloqueiam de verdade | K8, KA, SR, SE |
...
| Revisão periódica de permissões e de exposição | Atende | Roteiro em [revisão periódica](../operacional/revisao-periodica.md), com o prazo declarado em `.config/security-review.conf`, cobrado pelo job `secret-age` | LU, AC |
