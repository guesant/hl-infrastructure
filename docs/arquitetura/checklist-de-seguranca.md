...
| Pod Security Standards aplicados em todo namespace | Atende | `enforce`, `warn` e `audit` `restricted` em `blog`, `argocd`, `cert-manager`, `cnpg-system` e `sops`, por `managedNamespaceMetadata` e pela role `argocd`; `kube-system` fica sem enforce porque o Cilium e o k3s precisam de privilégio; o job `pod-security` exige o label em todo namespace novo | K8, KA, SR, SE |
...
| Benchmark CIS periódico (kube-bench) | Não atende | Nunca rodado; esta página é a revisão mais próxima disso | SR, SE |
