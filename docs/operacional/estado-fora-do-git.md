...
| token do k3s | `/var/lib/rancher/k3s/server/token` no node, com cópia cifrada em `node/k3s-token.sops.env` | o k3s na instalação; copiado por `just k3s-token-escrow` | a cópia cifrada basta para reconstruir a configuração de um segundo node ou recuperar o cluster; depois de `just rotate-token`, rode `just k3s-token-escrow` de novo |
