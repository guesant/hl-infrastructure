...
| credenciais PostgreSQL do blog e do Keycloak | Secrets A/B cifrados por SOPS em `argocd/apps/secrets/data/postgres/templates` e cópias locais criadas pelo ESO | `just postgres-rotation-prepare`, seguido de ativação GitOps | preparar o slot inativo, testar a conexão, ativar, observar por 30 minutos e aposentar o slot anterior |
