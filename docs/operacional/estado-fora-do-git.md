...
| passphrase do state do OpenTofu | cifrada no mesmo `cloudflare.sops.env`, e numa nota segura do gerenciador de senhas | o operador, no primeiro bootstrap | o `terraform.tfstate` commitado deixa de abrir; o túnel e o DNS continuam no ar, e uma passphrase nova mais `tofu import` dos três recursos reconstrói o state |
