...
4. **Faça o dry-run.** `just bootstrap-check -K` para o Ansible; `argocd app diff` ou `kubectl diff` para manifestos do Argo; `just infra-render-charts` seguido dos gates para versão de chart. O dry-run precisa mostrar exatamente a mudança esperada e nada mais; qualquer linha inesperada volta para a etapa 1.
...
6. **Aplique e observe a mudança real, não o status.** Depois de `just bootstrap -K` ou do sync do Argo, confirme o efeito pelo comportamento (o pod novo respondendo, a regra listada em `firewall-cmd --list-all`, o login SSH numa sessão nova), não por `changed=1` nem por `Synced`, que dizem que algo foi aplicado, não que funciona.

7. **Elimine o falso positivo.** Rode uma segunda vez: `just bootstrap -K` precisa terminar com `changed=0`, e `just bootstrap-check -K` sem nenhuma linha de diff. Se ainda há mudança, a role não é idempotente e a mudança não está terminada.
