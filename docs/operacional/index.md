...
A rotina de quem já conhece o repositório: implantar mudanças, manter o cluster e resolver tarefas específicas do dia a dia. Cada página aqui resolve uma tarefa concreta e assume familiaridade básica com Ansible, Helm e kubectl; para aprender os conceitos do zero, veja [Aprender](../aprender/index.md), e para entender por que uma peça funciona do jeito que funciona, veja a [arquitetura](../arquitetura/index.md). A lista abaixo começa pelo que acontece uma vez só, o bootstrap do node, e segue para o que se repete, a metodologia de toda mudança, os gates que rodam antes de uma pull request e as rotinas de rotação, restauração e revisão.
...
- [Renderizar os charts localmente](renderizar-charts-localmente.md): gera os manifestos Kubernetes que os charts Helm instalam, sem tocar em nenhum cluster real.
