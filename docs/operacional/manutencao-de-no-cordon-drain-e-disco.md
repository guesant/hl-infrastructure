...
Num cluster com vários servidores de control plane (o mecanismo de quorum está descrito em [Quorum](../aprender/kubernetes/control-plane/quorum.md)), drenar um servidor para manutenção o retira temporariamente do consenso, exatamente como se ele tivesse falhado.
...
[Diagnóstico de Pod, nó, certificado e Argo CD](diagnostico-de-pod-no-cluster-e-do-argocd.md) cobre o que fazer quando um nó já está `NotReady` em vez de em manutenção planejada. [Datastore do K3s](../aprender/kubernetes/control-plane/datastore.md) explica por que o datastore local de um nó é um dos dois consumidores de disco citados aqui.
