...
| `argocd/apps/platform/keycloak/templates/keycloak-realm.sops-secret.yaml` | `SopsSecret` | `tofu/keycloak-homelab` e `keycloak-management` (pelo `secrets.map`); nenhum pod o consome mais | `plan` e `apply` dos realms | e-mail do operador e credenciais do Google |
...
- `keycloak-homelab`: `state.sops.env`; pelo `secrets.map`, o segredo de `tofu-homelab` do `keycloak-master.sops.env`, o e-mail e o Google do `SopsSecret` `keycloak-realm`, e o segredo do client `blog` do `SopsSecret` `app-secret` do próprio blog. Não tem `.sops.env` próprio.
...
O que não é nem um nem outro, porque vive fora do git, está listado em [estado fora do git](estado-fora-do-git.md): a ACL da tailnet, a expiração da chave do node, o client OAuth no console do Google (usado só pelo Keycloak, que é quem fala com o Google; nenhuma aplicação tem mais login próprio pelo Google), as configurações do Portainer que o Job reaplica a cada sync, e as identidades privadas que decifram tudo.
