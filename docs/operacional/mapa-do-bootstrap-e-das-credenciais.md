...
A quarta são os serviços externos ao cluster que o OpenTofu declara: o túnel e o DNS na Cloudflare, o split DNS na tailnet. Eles rodam do Mac do operador, com Touch ID, e dependem da camada anterior só para o túnel ter para onde apontar; o DNS da tailnet depende de o node já estar nela (camada dois).

A quinta é o Keycloak, também pelo OpenTofu: os três realms e tudo dentro deles. Ela depende de todas as anteriores, porque fala com o Keycloak pelo nome interno da tailnet (camada dois), que o ingress serve (camada três) com o certificado da CA interna (camada três), e porque os client secrets que ela aplica são os mesmos `SopsSecret` que os consumidores da camada três montam.
...
- O ingress exige o cert-manager (a CA interna é um `ClusterIssuer`), a zona `tailscale` do firewall com `http` e `https`, e o sysctl que deixa o Traefik escutar em portas baixas sem root; os três vêm do Ansible.
- O login em qualquer serviço exige o Keycloak no ar (Argo) e o realm aplicado (Tofu); não há provedor externo. A senha e o TOTP do operador são cadastrados no primeiro login e vivem só no Keycloak.
...
Toda credencial que o repositório conhece está cifrada com SOPS para os destinatários de `.sops.yaml`: a identidade da Secure Enclave do operador, a chave do node e a chave de desastre. O que muda de um arquivo para outro é quem o lê e quando.
...
| `tofu/keycloak-master/keycloak-master.sops.env` | dotenv | `tofu-run.sh` para `keycloak-master`; os outros dois módulos leem dele as contas de serviço pelo `secrets.map` | `plan` e `apply` dos três módulos do Keycloak | administrador atual e permanente do `master`, segredos de `tofu-homelab` e `tofu-management`, senhas iniciais temporárias do operador nos dois realms |
...
- `keycloak-homelab`: `state.sops.env`; pelo `secrets.map`, o segredo de `tofu-homelab` e a senha inicial do operador, ambos do `keycloak-master.sops.env`, e o segredo do client `blog` do `SopsSecret` `app-secret` do próprio blog. Não tem `.sops.env` próprio.
- `keycloak-management`: `state.sops.env`; pelo `secrets.map`, o segredo de `tofu-management` e a senha inicial do operador do `keycloak-master.sops.env`, e os client secrets dos `SopsSecret` de `sso`, `oauth2-proxy` e `portainer`. Também sem `.sops.env` próprio.
...
| `TF_VAR_homelab_operator_initial_password`, `TF_VAR_management_operator_initial_password` | `keycloak-master.sops.env` | só bootstrap | senha temporária gravada ao criar o usuário e trocada por ele no primeiro login; mudar o valor depois não muda nada, e recuperar um acesso perdido é `taint` do usuário mais `apply` do realm, que o recria com a senha declarada |
...
O que não é nem um nem outro, porque vive fora do git, está listado em [estado fora do git](estado-fora-do-git.md): a ACL da tailnet, a expiração da chave do node, a senha e o TOTP do operador em cada realm (cadastrados no primeiro login, só no Keycloak), as configurações do Portainer que o Job reaplica a cada sync, e as identidades privadas que decifram tudo.
