...
O `Cluster` não declara `storageClass` porque a classe padrão do cluster é a única que existe, `local-path` com `Retain`, e não há outra a escolher.
