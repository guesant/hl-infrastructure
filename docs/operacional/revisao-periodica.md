...
7. Deriva do que vive fora do cluster: `just tofu-plan-all`, que roda `plan` em todo módulo de `tofu/` (`just tofu-list` mostra a lista completa, caso um módulo novo tenha entrado desde a última revisão); qualquer mudança que apareça foi feita à mão no console e precisa voltar ao git ou ser desfeita módulo a módulo com `just tofu <módulo> apply` (`just tofu-apply-all` aplica todos em sequência, mas sem respeitar a dependência entre `keycloak-master` e os outros dois módulos de Keycloak, então prefira o `apply` por módulo se a deriva envolver Keycloak). No mesmo espírito, `just bootstrap --check -e chart_reconcile=true` força as roles `cilium` e `argocd` a comparar o chart com o cluster, o que o atalho por hash não faz.
