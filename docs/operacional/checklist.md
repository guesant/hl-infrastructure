...
Lista curta para marcar a cada bootstrap, upgrade de versão ou mudança de rede. A regra é uma só: não marca sem conferir de verdade naquela execução. O estado ao lado de cada item reflete a última vez em que foi conferido num node real, e é atualizado no mesmo commit da mudança que motivou a execução.

| Item | Como conferir | Estado |
| --- | --- | --- |
| Preflight passou | `just preflight` termina sem `failed` | confirmado |
| Dry-run mostrou só a mudança esperada | `just bootstrap-check` com diff revisado linha a linha | confirmado |
| Segunda execução é idempotente | `just bootstrap` de novo termina com `changed=0` | pendente |
| SSH continua acessível por uma sessão nova | `ssh` novo depois de qualquer mudança em `firewall` ou `ssh_hardening` | confirmado |
...
| Node `Ready` e Cilium saudável | `just status` para o node, mais `cilium status` sem erro | confirmado |
...
| Timer de manutenção armado | `systemctl list-timers hl-gc.timer` mostra a próxima execução | confirmado |
...
| Login pelo Keycloak em cada aplicação | Dashy, Grafana, Portainer, Argo CD e `guesant.net/admin` entram com o usuário do realm e recusam um usuário fora do grupo `admins` | confirmado, exceto o blog, ainda sem usuário no realm `homelab` |
