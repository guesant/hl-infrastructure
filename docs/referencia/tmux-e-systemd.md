...
`screen` (GNU Screen) resolve o mesmo problema central, mais antigo que tmux; os comandos equivalentes usam o prefixo `Ctrl+a` em vez de `Ctrl+b` (`screen -S nome`, `Ctrl+a d` para desanexar, `screen -r nome` para reconectar). A diferença prática está em divisão de painéis (nativa e mais rica no tmux desde o início, tardia e mais limitada no screen) e em scriptability (o `tmux.conf` e comandos como `send-keys` facilitam automação de um jeito que o screen não acompanha tão bem). Vale escolher screen só quando ele já está instalado num host minimalista onde tmux não pode ser adicionado, ou quando a necessidade é só um detach/reattach simples sem painéis; fora disso, tmux é a escolha mais capaz para o mesmo trabalho.
