...
Um servidor adicional usa o par equivalente, `--server` e `--token`, descrito em [Quorum](kubernetes/control-plane/quorum.md#bootstrap-do-quorum). A diferença entre os dois papéis nesse momento de ingresso é a mesma de sempre: um entra no quorum, o outro só passa a executar workloads.
