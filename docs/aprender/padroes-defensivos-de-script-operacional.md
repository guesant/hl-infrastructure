...
## Criar um arquivo sensível já com a permissão certa, não corrigir depois

Escrever um arquivo com conteúdo sensível (um token, uma chave, um arquivo de configuração que os inclui) e só depois restringir sua permissão com um `chmod` separado deixa uma janela real, ainda que curta, em que o arquivo existe no disco com a permissão padrão, mais aberta do que deveria. `umask` muda essa permissão padrão para qualquer arquivo criado a partir daquele ponto do script, então declarar um `umask` restritivo imediatamente antes do comando que grava o arquivo sensível garante que ele nasce já com a permissão final, sem nenhuma janela intermediária mais permissiva para fechar depois. É uma inversão pequena de ordem, corrigir a permissão antes de escrever em vez de depois, mas ela elimina por completo o intervalo de tempo que a versão "escrever depois corrigir" sempre deixa aberto.
