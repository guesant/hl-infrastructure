...
| --- | --- | --- |
| Acesso inicial | Serviço remoto externo (`T1133`), força bruta (`T1110`) | Só SSH por chave e a API do k3s por CIDR ficam expostos na rede local; nada na Internet além do túnel Cloudflare, que só encaminha para o blog. fail2ban e o `bruteForceProtected` dos realms do Keycloak limitam tentativas. |
...
| Execução, persistência | Implantar contêiner (`T1610`), imagem maliciosa | Políticas de admissão exigem imagem por digest de um registry da lista, contêiner sem root, sem escalada de privilégio e sem capabilities; o Argo CD só aplica o que está em `main`, e o Image Updater só promove tags imutáveis já escaneadas. |
...
| Descoberta, movimento lateral | Descoberta de rede e serviços (`T1046`), movimento entre pods | Políticas de rede do Cilium por namespace com lista de liberação (ainda em modo auditoria, uma das pendências abertas), e a tailnet como única fronteira dos nomes internos. |
