...
| --- | --- |
| `ansible/site.yml` | playbook que encadeia as roles |
| `ssh_hardening` | role que descreve estado |
| `unattended_upgrades` | role que descreve estado |
| `firewall` | role que dispara um comando |
| `firewall-cmd` | comando disparado pela role `firewall`, cuja saída é interpretada |
...
Reduzindo isso ao que o hl-infrastructure de fato faz hoje, o [Ansible](ansible.md) parte de um Raspberry Pi que já existe fisicamente e prepara o sistema operacional e o cluster [k3s](k3s.md). A passagem de bastão está na própria ordem das roles de `ansible/site.yml`: a role argocd sobe o [Argo CD](argocd.md) e a role bootstrap_app, logo em seguida, registra a aplicação raiz que ele vai sincronizar.

| Role | Função |
| --- | --- |
| `argocd` | sobe o Argo CD |
| `bootstrap_app` | registra a aplicação raiz que ele vai sincronizar |

Daí em diante o Argo CD reconcilia continuamente o estado do cluster a partir deste mesmo repositório, sem depender do Ansible rodar de novo para isso. O Ansible volta a ser necessário só quando muda algo abaixo do cluster, no sistema operacional do node.
...
O critério de divisão, portanto, não é a prática nem a preferência por uma ferramenta, é onde o recurso mora. O que existe dentro do cluster cabe ao Argo CD, o que existe no sistema operacional do node cabe ao Ansible, e o que existe numa conta de serviço de terceiro só é alcançável pela API dessa conta.
