...
CI/CD é geralmente pronunciado como um termo só, mas na verdade descreve práticas distintas, com fronteiras claras entre elas, e a confusão entre entrega contínua e implantação contínua é comum mesmo entre quem trabalha com isso todo dia.
...
**Implantação contínua** (Continuous Deployment) remove esse botão: toda mudança que passa pelas verificações automatizadas vai para produção sem intervenção humana nenhuma. A diferença entre entrega e implantação contínua não é técnica, é de decisão organizacional: ambas exigem a mesma maturidade de automação e teste, mas só uma delas remove a aprovação humana do caminho crítico.
