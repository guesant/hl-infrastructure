...
[A pipeline de CI](../arquitetura/ci.md), na arquitetura, descreve os 16 jobs paralelos que compõem a integração contínua deste repositório e por que ele para exatamente nesse ponto, sem implantação automática de si mesmo (o que muda automaticamente, via ArgoCD, é o estado do cluster a partir do git, não o próprio processo de CI). ["GitOps: root e satélites"](../arquitetura/gitops-root-e-satelites.md) explica como a convergência contínua do cluster se relaciona, e se diferencia, desses três conceitos.
