...
while IFS= read -r image; do
  ...
  name="$(tr '/:@' '___' <<<"$image")"
  echo "== $image"
  run_trivy image --quiet --platform "$platform" --format cyclonedx --output "$sbom_dir/$name.cdx.json" "$image"
  if ! run_trivy image --quiet --platform "$platform" --severity CRITICAL --ignore-unfixed --ignorefile .trivyignore.yaml --exit-code 1 "$image"; then
    status=1
  fi
done <"$images_file"
