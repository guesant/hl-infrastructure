...
for dir in tofu/*/; do
    ...
    docker run --rm -v "$work":/work -w /work -e TF_VAR_state_passphrase "$TOFU_IMAGE" -chdir="$name" init -backend=false -input=false
    docker run --rm -v "$work":/work -w /work -e TF_VAR_state_passphrase "$TOFU_IMAGE" -chdir="$name" validate
done
